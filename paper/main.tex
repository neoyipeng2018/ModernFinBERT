...
\begin{table}[h]
\centering
\caption{Training data provenance. All non-FPB labels are LLM-generated via prompted classification; FPB labels are from human financial professionals.}
...
Three characteristics of this corpus deserve attention. First, all non-FPB labels are \textbf{LLM-generated} via prompted classification (``Classify the sentiment of this [type]...''), not human-annotated. While this enables scalable dataset construction, it introduces potential LLM annotation artifacts---notably, Source~4 (press releases) has only 3.4\% NEGATIVE labels, substantially below the 12.5\% rate in human-annotated FPB, suggesting a positivity bias in the LLM labeler. Second, Source~4 is dominated by \textbf{Canadian mining press releases} (68\% of samples mention TSX, mining, or resource-extraction terms), making it a narrow sub-domain rather than a representative press release sample. Third, Source~8 (earnings call Q\&A) has a \textbf{median length of 161 words}, meaning a substantial fraction of these samples are truncated at the 512-token limit during training---the model learns from incomplete texts for this source.
...
This 6.4 percentage point ``protocol gap'' arises because in-domain CV allows the model to train on FPB-distributed data, while the held-out setting tests true generalization. Our data provenance audit (Table~\ref{tab:data-provenance}) reveals a contributing factor: the training data has a NEGATIVE class rate of 10.2\%, below FPB's 12.5\%, creating a distribution shift that partially explains the held-out performance drop. We argue that reporting both protocols is essential for honest evaluation, and that the held-out number better reflects real-world deployment performance where the model encounters novel financial text.
...
Two factors likely contribute. First, the \textbf{domain mismatch}: Twitter financial text is informal, abbreviated, and opinionated, while FPB consists of formal press-release sentences. Our data provenance audit (Table~\ref{tab:data-provenance}) confirms this gap---the training data already derives 48\% of samples from tweets (Source~9, median 15 words) and earnings calls (Source~8, median 161 words), domains that differ substantially from FPB's press-release headlines (median 21 words). Adding more tweets via pseudo-labeling amplifies this mismatch. Second, the \textbf{overconfident teacher}: with mean and minimum confidence both near 1.0, the selected pseudo-labels carried no meaningful uncertainty signal, effectively amplifying the teacher's existing biases rather than adding complementary information.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{LLM-annotated training data}: All non-FPB training labels are LLM-generated, not human-annotated. This introduces potential systematic biases---for instance, the LLM labeler assigns NEGATIVE labels to only 3.4\% of press releases (Source~4), compared to 12.5\% in human-annotated FPB. The impact of LLM label noise on downstream model quality deserves further study.
    \item \textbf{Narrow sub-domain in training data}: Source~4 (press releases) is 68\% Canadian mining announcements (TSX-V listings, drill results, assay values), making it a narrow sub-domain rather than representative financial press releases. This may inflate apparent generalization to mining text while limiting transfer to other press release types.
    \item \textbf{Training data composition}: The aggregated training corpus combines heterogeneous sources with text lengths spanning 15 to 161 median words. The interaction between source diversity and model architecture deserves further study.

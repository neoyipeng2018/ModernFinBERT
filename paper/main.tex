...
\begin{abstract}
We present a systematic empirical study applying ModernBERT, a recent modernized BERT architecture, to financial sentiment analysis. Through eight controlled experiments, we evaluate ModernBERT-base with LoRA fine-tuning across multiple evaluation protocols on the FinancialPhraseBank (FPB) benchmark. Our study makes several contributions: (1) we introduce a held-out evaluation protocol where FPB is entirely excluded from training, providing a stricter generalization test than standard in-domain splits; (2) we propose DataBoost, a targeted augmentation method that uses LLM-generated paraphrases of misclassified samples to improve performance; (3) we provide controlled comparisons between ModernBERT and BERT-base, finding that ModernBERT consistently matches or outperforms BERT---leading by 1.09 percentage points in 10-fold CV ($p=0.093$) and by 7.84 points in the held-out setting on identical data---overturning an earlier spurious finding caused by a data leakage bug; and (4) we compare fine-tuned models against zero-shot Claude Opus 4.5, demonstrating that even modest fine-tuned models are orders of magnitude more cost-effective. Our results highlight the importance of evaluation protocol choice and proper experimental controls in financial NLP.
\end{abstract}
...
In this work, we conduct a systematic empirical study of ModernBERT for financial sentiment analysis. Rather than simply reporting a single accuracy number, we design eight complementary experiments that together provide a nuanced picture of model performance:
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Pre-trained baselines} (NB05): Compare ModernFinBERT against published FinBERT models.
    ...
    \item \textbf{Controlled replication} (NB09): Data contamination audit, head-to-head cross-validation, and controlled held-out comparison to isolate true architectural effects.
\end{enumerate}

A key finding is that evaluation protocol choice dramatically affects reported performance: the same ModernBERT model achieves 86.88\% under 10-fold CV on FPB but only 80.44\% when FPB is held out entirely. Additionally, our controlled replication study reveals that an initial finding of BERT outperforming ModernBERT by 14.75 points was caused by a data leakage bug; when properly controlled, ModernBERT consistently matches or outperforms BERT. We argue that the held-out protocol provides a more realistic estimate of real-world generalization and that the financial NLP community should adopt it as a complementary evaluation standard.
...
We evaluate on a diverse 723-sample financial sentiment test set spanning FinancialPhraseBank sentences, earnings call Q\&A transcripts, social media posts, press releases, and mining/resource announcements.
...
\begin{table}[h]
\centering
\caption{Fine-tuned ModernFinBERT vs.\ Claude Opus 4.6 with financial sentiment skill. ModernFinBERT evaluated on FPB sentences\_50agree; Claude evaluated on a diverse 723-sample financial test set including FPB sentences.}
\label{tab:claude}
\begin{tabular}{lccrr}
\toprule
...
Claude Opus 4.6 + skill & 0.7280 & 0.7030 & $\sim$\$0.008 & $\sim$1--2s \\
\midrule
...
Claude Opus 4.6 with the financial sentiment skill achieves 72.8\% accuracy and 0.703 macro F1---a substantial improvement over na\"ive zero-shot prompting but still 7.6 percentage points below the fine-tuned ModernFinBERT. Per-class analysis reveals that the POSITIVE $\leftrightarrow$ NEUTRAL boundary is the dominant confusion source (131 of 197 errors), while NEGATIVE recall is strongest at 0.819. Despite deploying a carefully engineered prompt with domain-specific classification rules, the fine-tuned 149M-parameter model remains approximately 800$\times$ cheaper and 500--1000$\times$ faster per inference, underscoring the continued value of task-specific fine-tuning for high-volume financial applications.
...
\subsection{Experiment 5: Pre-Trained FinBERT Baselines}
\label{sec:baselines}

\paragraph{Protocol.}
We evaluate two publicly available FinBERT models on FPB to establish reference baselines:
\begin{enumerate}[nosep]
    \item \textbf{ProsusAI/finbert}: Pre-trained FinBERT evaluated on FPB. This model was trained \emph{on} FPB data, so this is effectively in-domain evaluation.
    \item \textbf{yiyanghkust/finbert-tone}: FinBERT variant trained on analyst reports, evaluated zero-shot on FPB.
\end{enumerate}

\paragraph{Results.}
Table~\ref{tab:baselines} presents these baselines alongside our ModernFinBERT from Experiment~1. The controlled comparison between ModernBERT and BERT-base under identical conditions is deferred to Experiment~8 (Section~\ref{sec:controlled-replication}).

\begin{table}[h]
\centering
\caption{Pre-trained FinBERT baselines compared to ModernFinBERT on FPB.}
\label{tab:baselines}
...
ModernFinBERT's held-out accuracy (80.44\%) is lower than ProsusAI/finbert's in-domain result (88.96\%), as expected given that our model never sees FPB during training. On \texttt{sentences\_allAgree}, however, the gap narrows substantially (92.98\% vs.\ 97.17\%), indicating that ModernFinBERT handles unambiguous financial text well even without FPB exposure.
...
\subsection{Experiment 8: Controlled Replication Study}
\label{sec:controlled-replication}

\paragraph{Motivation.}
An earlier version of our experiments (not reported here) appeared to show BERT-base outperforming ModernBERT by 14.75 percentage points in the held-out setting. However, we discovered a data leakage bug: the source ID filter intended to exclude FPB from training was using the wrong value (\texttt{source\_id $\neq$ 6} instead of \texttt{$\neq$ 5}), causing FPB samples to leak into the training set for those runs. We conduct three sub-experiments with verified-clean data to establish the true architectural comparison.

\paragraph{Experiment 8a: Data Contamination Audit.}
Before comparing architectures, we verify that the aggregated training data contains no FPB samples that could inflate held-out results. We apply three progressively sensitive checks: (1) exact string matching between all training samples and FPB, (2) fuzzy matching using SequenceMatcher with a threshold of 0.90, and (3) semantic similarity using sentence-transformer embeddings with a cosine threshold of 0.95. All three checks return \textbf{zero matches}, confirming the training data is clean and that no data leakage contributes to the held-out evaluation results.

\paragraph{Experiment 8b: Head-to-Head 10-Fold Cross-Validation.}
We run stratified 10-fold CV on FPB \texttt{sentences\_50agree} for both BERT-base and ModernBERT under identical LoRA $r=16$ configurations (batch size 16, learning rate $2 \times 10^{-4}$, cosine schedule, 10 epochs, FP16). This eliminates any training data confound since both models train and evaluate on the same FPB folds.
...
ModernBERT wins 7 of 10 folds, with a mean advantage of +1.09 percentage points. A paired $t$-test yields $t = 1.88$, $p = 0.093$---not statistically significant at $p < 0.05$ but marginally so, suggesting a trend in ModernBERT's favor. Importantly, there is \emph{no evidence} that BERT outperforms ModernBERT when both see identical data.

\paragraph{Experiment 8c: Controlled Held-Out Evaluation.}
We retrain both models on the \emph{same} 8,643-sample aggregated dataset (the original version used for the ModernBERT experiments) under identical hyperparameters (batch size 8, gradient accumulation 4, learning rate $2 \times 10^{-4}$, cosine schedule, 10 epochs, seed 3407).

\begin{table}[h]
\centering
\caption{Controlled architecture comparison on FPB (held-out, FPB excluded from training). Both LoRA models trained on identical 8,643 samples with identical hyperparameters. Reference baselines from Experiment~5 included for context.}
\label{tab:controlled-holdout}
\begin{tabular}{llcccc}
\toprule
\textbf{Model} & \textbf{Protocol} & \multicolumn{2}{c}{\textbf{FPB 50agree}} & \multicolumn{2}{c}{\textbf{FPB allAgree}} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
...
\midrule
\multicolumn{6}{l}{\emph{Reference baselines (Experiment~5):}} \\
ProsusAI/finbert & In-domain$^\dagger$ & 0.8896 & 0.8825 & 0.9717 & 0.9625 \\
finbert-tone & Zero-shot & 0.7914 & 0.7530 & 0.9152 & 0.8939 \\
\midrule
\multicolumn{6}{l}{\emph{Controlled comparison (identical 8,643-sample training set):}} \\
ModernBERT + LoRA & Held-out & \textbf{0.8093} & \textbf{0.7793} & \textbf{0.9329} & --- \\
BERT-base + LoRA & Held-out & 0.7309 & 0.6051 & 0.8366 & --- \\
\midrule
$\Delta$ (MB $-$ BERT) & & \textbf{+0.0784} & \textbf{+0.1742} & \textbf{+0.0963} & \\
\bottomrule
\multicolumn{6}{l}{\small $^\dagger$ Trained on FPB data (in-domain evaluation).}
\end{tabular}
\end{table}

On identical training data, \textbf{ModernBERT outperforms BERT-base by 7.84 percentage points} on FPB \texttt{sentences\_50agree} and 9.63 points on \texttt{sentences\_allAgree}. ModernBERT also approaches ProsusAI/finbert (80.93\% vs.\ 88.96\%) despite never seeing FPB during training, while BERT-base falls well short (73.09\%).
...
Our controlled replication (Experiment~8) overturns the initial finding that BERT outperforms ModernBERT. The apparent 14.75-point gap was caused by a source ID filtering bug that allowed FPB to leak into the BERT training set. When properly controlled on verified-clean data:

\begin{itemize}[nosep]
    \item \textbf{Held-out evaluation}: ModernBERT outperforms BERT by 7.84 points on identical 8,643-sample training data (Table~\ref{tab:controlled-holdout}).
    \item \textbf{10-fold CV}: ModernBERT leads by 1.09 points, $p = 0.093$ (Table~\ref{tab:head-to-head-cv}).
\end{itemize}

Across both evaluation protocols, ModernBERT is consistently competitive with or superior to BERT-base. The held-out advantage (+7.84 points) suggests that ModernBERT's modernized architecture---rotary positional embeddings, Flash Attention, and GeGLU activations---provides genuinely stronger representations for financial text, even without domain-specific pre-training. This is encouraging for practitioners, as it suggests that upgrading from BERT to ModernBERT yields improvements on domain-specific tasks without requiring additional pre-training.

This finding also serves as a cautionary tale about the importance of data pipeline verification. The original confound---a single wrong constant in a filter---produced a compelling but entirely spurious result that would have led to incorrect conclusions about architecture design.
...
\begin{itemize}[nosep]
    \item \textbf{Accuracy}: ModernFinBERT (80.44\%) outperforms Claude Opus 4.6 with a carefully engineered financial sentiment skill (72.8\%) by 7.6 percentage points.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{LoRA only}: Full fine-tuning could yield different relative performance between architectures, particularly since ModernBERT's fused QKV projections receive effectively lower per-component rank than BERT's separate Q, K, V projections under the same LoRA rank.
...
We have presented a systematic study of ModernBERT for financial sentiment analysis, encompassing eight controlled experiments. Our key findings are:
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Data pipeline verification is essential}: An initial finding that BERT outperforms ModernBERT by 14.75 points was entirely caused by a data leakage bug (wrong source ID filter). Controlled experiments on verified-clean data show ModernBERT consistently matches or outperforms BERT.
    ...
    \item \textbf{Fine-tuning beats skill-prompted LLMs}: Even modest fine-tuned models outperform state-of-the-art LLMs equipped with purpose-built classification skills on structured sentiment tasks, while being orders of magnitude cheaper and faster.

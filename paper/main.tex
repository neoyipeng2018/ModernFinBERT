...
\paragraph{Protocol.}
We train ModernBERT-base with \emph{all} 149M parameters unfrozen (no LoRA) using hyperparameters adapted for full fine-tuning: learning rate $2 \times 10^{-5}$, weight decay 0.01, warmup ratio 0.1, and effective batch size 16. We run two conditions: (1) baseline training on the aggregated dataset, and (2) training on the same data augmented with the 410 VS-CoT DataBoost samples from Experiment~2. This produces a 2$\times$2 comparison: \{LoRA, Full FT\} $\times$ \{Baseline, DataBoosted\}.
